...
from .feature import TimeSurface, Event, remove_isolated_pixels
from .util import cosine_dist, euclidean_dist
from .NmnistClass import NmnistDataset
...
def visualise_time_surface_for_event_stream(N, tau, r, width, height, events):
    ...
    event_data_filt, _, _ = remove_isolated_pixels(events, eps=3, min_samples=20)
...
def initialise_time_surface_prototypes(N, tau, r, width, height, events, init_method=6, plot=False):
...
def generate_histogram(file_name, C_1, C_2, C_3, N_1, N_2, N_3, r_1, r_2, r_3, tau_1, tau_2, tau_3, use_all_events):
    ...
    ev = NmnistDataset.read_dataset(file_name)
    ev_data = ev.data
