...
def remove_isolated_pixels(events, eps=3, min_samples=20):
    
    coords = np.vstack((events.x, events.y)).T
    nbrs = NearestNeighbors(n_neighbors=min_samples, radius=eps).fit(coords)
    distances, indices = nbrs.kneighbors(coords)

    mask = (distances[:, -1] <= eps)
